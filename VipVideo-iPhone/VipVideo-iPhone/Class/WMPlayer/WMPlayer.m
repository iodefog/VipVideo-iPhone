...
//控制全屏和播放暂停按钮
@property (nonatomic,retain) UIButton  *fullScreenBtn,*playOrPauseBtn,*lockBtn,*backBtn,*downloadBtn;
//进度滑块&声音滑块
...
-(void)initWMPlayer{
    ...
    [self.downloadBtn addTarget:self action:@selector(downloadTheVideo:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)addUIControlConstraints{
    ...
    [self.downloadBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.size.mas_equalTo(CGSizeMake(self.downloadBtn.currentImage.size.width+6, self.backBtn.currentImage.size.height+4));
        make.centerY.equalTo(self.titleLabel);
    }];
    [self.titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.leading.equalTo(self.backBtn.mas_trailing).offset(50);
...
-(void)downloadTheVideo:(UIButton *)sender{
    if (self.delegate&&[self.delegate respondsToSelector:@selector(wmplayer:clickedDownloadButton:)]) {
        [self.delegate wmplayer:self clickedDownloadButton:sender];
    }
}
#pragma mark
#pragma mark - 关闭按钮点击func
    ...
    if (context == PlayViewStatusObservationContext){
        if ([keyPath isEqualToString:@"status"]) {
            ...
            if ((CGFloat)CMTimeGetSeconds(self.currentItem.duration) != self.totalTime) {
                ...
                self.progressSlider.maximumValue = isnan(self.totalTime)?0:self.totalTime;
                if (self.state==WMPlayerStateStopped||self.state==WMPlayerStatePause) {
